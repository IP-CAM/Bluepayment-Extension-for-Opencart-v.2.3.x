...
class ModelExtensionPaymentBluepaymentVisa extends Model
{
    ...
    {
        // Verify zone
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('bluepayment_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
        if ($this->config->get('bluepayment_total') > 0 && $this->config->get('bluepayment_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('bluepayment_geo_zone_id')) {
            $status = true;
        ...
        return [
            ...
            'sort_order' => $this->config->get('bluepayment_sort_order'),
            'terms' => '',
        ];
    }
}
